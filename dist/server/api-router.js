...
const SUPPORTED_VERBS = ['GET', 'POST', 'PUT', 'PATCH', 'DELETE', 'OPTIONS', 'HEAD'];
...
export function scanApiRoutes(appDir) {
    if (!fs.existsSync(appDir) || !fs.statSync(appDir).isDirectory())
        return [];
    const out = [];
    const stack = [{ dir: appDir, rel: '' }];
    while (stack.length) {
...
export async function dispatchApi(request, compiled, loadModule) {
    ...
    for (const r of compiled) {
        ...
        const mod = (await loadModule(r.modulePath));
