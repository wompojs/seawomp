...
function SeawompLink(props) {
    const { children, href, locale: localeOverride, follow, target, rel, download, title, ariaLabel, ariaCurrent, } = props;
    ...
    const resolvedHref = effectiveHref !== undefined ? resolveHref(effectiveHref, localeOverride) : undefined;
    if (!IS_SERVER) {
    ...
    const effectiveAriaCurrent = ariaCurrent !== undefined ? ariaCurrent : resolveAriaCurrent(resolvedHref);
...
function setupClientBehavior(props) {
    ...
    useEffect(() => {
        if (follow !== 'path' || typeof window === 'undefined')
            return;
    ...
    useEffect(() => {
        ...
        const shouldPreloadModules = normalizeBoolean(preloadModules ?? self.getAttribute('preload-modules'), true);
        let hoverTimer;
        const cancelHover = () => {
        ...
        const prefetchAnchor = () => {
            ...
            if (!href || !isLocalNavigableHref(href))
                return;
            hoverTimer = window.setTimeout(() => prefetchRoute(href, { preloadModules: shouldPreloadModules }), delay);
        };
        const click = (event) => {
            const href = anchor.getAttribute('href');
            if (!href || !isLocalNavigableHref(href))
        ...
        const syncActive = () => {
            ...
            else if (anchor.hasAttribute('data-seawomp-active-managed') ||
...
function resolveHref(href, localeOverride) {
...
function currentPathnameForLink() {
    if (IS_SERVER)
        return undefined;
    return stripLocaleFromPathname(window.location.pathname);
}
function normalizePrefetchMode(value) {
...
function isLocalNavigableHref(href) {
...
function resolveAriaCurrent(resolvedHref) {
    if (resolvedHref === undefined || !resolvedHref.startsWith('/'))
        return undefined;
    if (IS_SERVER) {
