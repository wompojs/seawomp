...
export async function buildAll(cfg, cwd, opts = {}) {
    ...
    const hydrateEntrySource = generateHydrateEntrySource(routes, cfg.i18n, cfg.navigation, clientEntryMap, specialRoutes);
    ...
    await rewriteHydrateRuntimeImports(path.join(staticDir, hydrateRuntime.replace(/^\//, '')), clientEntryMap, clientAssetByEntry);
    ...
    const transformHtml = async (html) => postProcessHtml(await localizeGoogleFontsInHtml(html, fontContext), {
    ...
    if (ssg.written.length)
        console.log(`[seawomp] prerendered ${ssg.written.length} page(s)`);
    ...
    if (fontContext.written)
        console.log(`[seawomp] localized ${fontContext.written} font asset(s)`);
...
async function cleanBuildOutput(outDir) {
    await Promise.all(['static', 'server', 'client-entries'].map((entry) => removePath(path.join(outDir, entry))));
}
...
async function writeClientEntryProxies(outDir, routes, specialRoutes) {
...
function mapClientEntryOutputs(outputs, entrypoints) {
    const byEntry = new Map();
    for (const entry of entrypoints) {
        const stem = path.basename(entry, path.extname(entry));
        const output = outputs.find((o) => o.kind === 'entry-point' && path.basename(o.path).startsWith(`${stem}-`));
        if (output)
            byEntry.set(entry, '/_assets/' + path.basename(output.path));
    }
    return byEntry;
}
async function rewriteHydrateRuntimeImports(hydratePath, sourceToProxy, proxyToAsset) {
...
function specialRouteToManifest(cwd, outDir, serverDir, route) {
    return {
        page: route.pagePath,
        layouts: route.layoutPaths,
        serverPage: toOutDirRelative(outDir, mapToServerBundle(cwd, serverDir, route.pagePath)),
        serverLayouts: route.layoutPaths.map((p) => toOutDirRelative(outDir, mapToServerBundle(cwd, serverDir, p))),
    };
}
function mapRouteToServerRoute(cwd, serverDir, route) {
    return {
        ...route,
...
function mapSpecialRoutesToServer(cwd, serverDir, specialRoutes) {
    return {
...
function mapSpecialRouteToServer(cwd, serverDir, route) {
    if (!route)
        return undefined;
    return {
        pagePath: mapToServerBundle(cwd, serverDir, route.pagePath),
...
function composeFrameworkHead(manifest, frameworkHead) {
...
async function importFile(abs) {
    return import(pathToFileURL(abs).href);
}
async function renderStaticNotFound(opts) {
    if (!opts.specialRoutes.notFoundRoute)
        return;
...
function generateHydrateEntrySource(routes, i18n, navigation, clientEntryMap, specialRoutes) {
...
function specialClientRecord(route, resolve) {
    if (!route)
        return null;
    return {
