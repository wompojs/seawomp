...
export async function buildImages(opts) {
    ...
    if (!sharp) {
        console.warn('[seawomp] skipped image compression — `sharp` is not installed (originals will be served as-is, no AVIF/WebP variants or srcset). Run `bun add sharp` to enable it.');
    }
    let written = 0;
    // ── Local images ──────────────────────────────────────────────────────────
