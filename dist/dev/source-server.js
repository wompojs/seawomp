...
const srcKnownPaths = new Set();
export function invalidateSrc(abs) {
    if (abs)
        srcCache.delete(abs);
    else
        srcCache.clear();
}
export function invalidateDeps() {
    depCache.clear();
}
/** Try several extensions for an import like `./foo` that omitted one. */
async function resolveRelative(fromDir, spec) {
...
const IMPORT_RE = /((?:^|[^.\w$])(?:import\s*(?:[\w*${}\s,]+\s+from\s+)?|export\s+(?:[\w*${}\s,]+\s+from\s+)?|import\s*))(['"])([^'"\n]+)\2/g;
const DYNAMIC_IMPORT_RE = /(\bimport\s*\(\s*)(['"])([^'"\n]+)\2(\s*\))/g;
async function rewriteImports(jsCode, fromFileAbs, cwd) {
    const fromDir = path.dirname(fromFileAbs);
    async function rewrite(spec) {
        // Relative path
    ...
    for (const match of matches) {
        ...
        const replacement = match.full.replace(match.quote + match.spec + match.quote, match.quote + rewritten + match.quote);
...
export async function serveSrc(absPath, cwd) {
    let st;
    try {
    ...
    if (TS_EXTS.has(ext) || ext === '.tsx') {
        ...
        const transpiler = new Bun.Transpiler({ loader: loader, target: 'browser' });
        js = await transpiler.transform(raw, loader);
    }
...
const pkgOf = (s) => s.startsWith('@') ? s.split('/').slice(0, 2).join('/') : s.split('/', 1)[0];
...
export async function serveDep(spec, cwd) {
    const cached = depCache.get(spec);
    if (cached)
        return { code: cached, type: 'application/javascript' };
...
function rewriteSingletonImports(code, currentPkg) {
    const re = /((?:from|import)\s*\(?\s*)(['"])(wompo|seawomp)((?:\/[^'"]*)?)\2/g;
    return code.replace(re, (full, lead, quote, pkg, sub) => {
        if (pkg === currentPkg)
            return full;
...
async function serveStaticSingleton(spec, cwd) {
    ...
    if (!absPath.startsWith(pkgRoot + path.sep))
        return null;
    let raw;
    try {
    ...
    const rewritten = raw.replace(/(?<=^|[^.\w$])(from|import)\s*(\(\s*)?(['"])([^'"\n]+)\3/g, (full, kw, paren, quote, importPath) => {
