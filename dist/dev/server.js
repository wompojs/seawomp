...
export async function startDev(cfg, cwd) {
    let routes = scanRoutes(cfg.appDir);
    let apiRoutes = scanApiRoutes(cfg.appDir);
    // Cached head fragment: read globalCss off disk and stitch it together with headExtra.
    let cachedHeadExtra = null;
    async function computeHeadExtra() {
        if (cachedHeadExtra !== null)
        ...
        if (cfg.globalCss) {
            try {
                ...
                console.warn(`[seawomp] could not read globalCss at ${cfg.globalCss}: ${err.message}`);
            }
        }
