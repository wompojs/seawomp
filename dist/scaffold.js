...
export async function scaffoldProject(opts) {
    ...
    if (!opts.skipInstall) {
        ...
        if (code !== 0) {
            console.warn(`[seawomp] bun install exited with code ${code} — finish manually with \`cd ${path.basename(dir)} && bun install\`.`);
        }
    }
