...
export async function loadBuildManifest(cfg: ResolvedConfig): Promise<BuildManifest> {
	...
	try {
		...
		throw new Error(
			`[seawomp] could not read manifest at ${cfg.outDir}/manifest.json — did you run \`seawomp build\`?`,
		);
	}
	return manifest;
}

export async function createProdHandler(
	cfg: ResolvedConfig,
	cwd: string,
	...
	return async (req) => {
		...
		const ssgHtml = await serveStatic(
			staticDir,
			pathname === '/' ? '/index.html' : pathname + '/index.html',
			{ request: req, mode: 'prod' },
		);
		if (ssgHtml) return ssgHtml;
...
export async function startProd(cfg: ResolvedConfig, cwd: string): Promise<void> {
...
function resolveServerRoutes(
	manifest: BuildManifest,
	cfg: ResolvedConfig,
	cwd: string,
): { routes: RouteEntry[]; apiRoutes: ApiRouteEntry[] } & SpecialRoutes {
	if (
		manifest.routes.some((r) => r.serverPage) ||
		...
		return {
			routes: manifest.routes.map((r) => ({
				pattern: r.pattern,
				pagePath: resolveOutDirPath(
					cfg.outDir,
					r.serverPage ?? mapToServerBundle(cwd, cfg.outDir, r.page),
				),
				layoutPaths: (
					r.serverLayouts ?? r.layouts.map((p) => mapToServerBundle(cwd, cfg.outDir, p))
			...
			apiRoutes: manifest.apiRoutes.map((r) => ({
				pattern: r.pattern,
				modulePath: resolveOutDirPath(
					cfg.outDir,
					r.serverModulePath ?? mapToServerBundle(cwd, cfg.outDir, r.modulePath),
				),
			})),
...
function resolveSpecialRoute(
	route: BuildManifest['notFoundRoute'],
	cfg: ResolvedConfig,
	cwd: string,
...
function resolveOutDirPath(outDir: string, p: string): string {
	return path.isAbsolute(p) ? p : path.join(outDir, p);
}

function mapToServerBundle(cwd: string, outDir: string, abs: string): string {
