 ...
 *   // → 'en'
 */
export function getLocale(url: URL, config: I18nConfig): string {
...
export function hasLocalePrefix(pathname: string, config: I18nConfig): boolean {
...
export function seoI18nHead(options: SeoI18nHeadOptions): RenderHtml {
	const locale =
		options.locale ??
...
function toOgLocale(locale: string, map: Record<string, string> | undefined): string {
 ...
 */
export async function loadMessages(locale: string, messagesDir: string): Promise<Messages> {
...
function flattenMessages(
	...
	for (const [key, val] of Object.entries(obj)) {
		...
		if (val && typeof val === 'object' && !Array.isArray(val)) {
			Object.assign(out, flattenMessages(val as Record<string, unknown>, full));
		} else {
			out[full] = String(val);
		}
	}
