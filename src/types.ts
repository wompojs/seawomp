...
export type Loader<T = unknown, P extends Record<string, string> = Record<string, string>> = (
	args: LoaderArgs<P>,
) => Promise<T> | T;
...
/* API route handlers — exported as named functions (GET, POST, …) from `app/api/**\/route.ts`. */
...
export type ApiHandler<P extends Record<string, string> = Record<string, string>> = (
