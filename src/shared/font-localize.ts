...
export function stripGoogleFontPreconnects(html: string): string {
	return html.replace(
 ...
 * manifest font map. */
export function findGoogleFontLinks(html: string): GoogleFontLink[] {
	const out: GoogleFontLink[] = [];
...
export function localFontLinkTag(localHref: string): string {
...
export function localizeGoogleFontsWithMap(
	html: string,
	map: Record<string, string> | undefined,
): string {
	if (!map || !html.includes('fonts.googleapis.com')) return html;
